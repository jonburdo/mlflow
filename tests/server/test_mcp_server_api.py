...
def test_version_crud_with_slashed_name(client):
    ...
    r = client.post(PREFIX + f"/{name}/versions", json={"server_json": sj})
    assert r.status_code == 200
    assert r.json()["name"] == name
    assert r.json()["version"] == "1.0"

    r = client.get(PREFIX + f"/{name}/versions/1.0")
    assert r.status_code == 200
    assert r.json()["version"] == "1.0"
...
def test_get_latest_version(client):
    for v in ["1.0", "2.0"]:
        sj = _server_json("lat", v)
        client.post(PREFIX + "/lat/versions", json={"server_json": sj, "status": "active"})
    r = client.get(PREFIX + "/lat/versions/latest")
    assert r.status_code == 200
    assert r.json()["version"] == "2.0"
...
def test_invalid_transport_type(client):
    ...
    assert r.status_code == 400
    assert "Invalid transport_type" in r.json()["detail"]
