...
def _resolve_binding_tools(binding: MCPAccessBinding) -> list[MCPToolPayload] | None:
    if binding.resolved_version is None or binding.resolved_version.tools is None:
        return None
    return [MCPToolPayload(**t.to_dict()) for t in binding.resolved_version.tools]
...
def create_mcp_server(request: CreateMCPServerRequest) -> MCPServerResponse:
    ...
    try:
        ...
        return _mlflow_error_response(e)
...
def search_all_access_bindings(
    ...
    try:
        ...
        bindings = [
            MCPAccessBindingResponse.from_entity(b, _resolve_binding_tools(b)) for b in results
        ]
...
def create_mcp_access_binding(
    ...
    try:
        ...
        return MCPAccessBindingResponse.from_entity(binding, _resolve_binding_tools(binding))
    except MlflowException as e:
        return _mlflow_error_response(e)
