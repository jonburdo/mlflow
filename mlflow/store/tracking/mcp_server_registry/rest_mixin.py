...
def _tool_from_dict(d: dict[str, Any]) -> MCPTool:
    return MCPTool(
        name=d["name"],
        title=d.get("title"),
...
def _tool_to_dict(t: MCPTool) -> dict[str, Any]:
    return t.to_dict()
...
class RestMCPServerRegistryMixin:
    ...
    def update_mcp_server_version(
        ...
        if tools is not NOT_SET:
            body["tools"] = None if tools is None else [_tool_to_dict(t) for t in tools]
        data = self._mcp_request(
